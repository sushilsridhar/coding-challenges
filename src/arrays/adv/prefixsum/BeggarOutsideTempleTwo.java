...
/*
       ...
       Example:Input:N = 5, D = [[1, 2, 10], [2, 3, 20], [2, 5, 25]]
       Return: [10, 55, 45, 25, 25]

       Explanation:=> First devotee donated 10 coins to beggars ranging from 1 to 2.
 ...
 */
public class BeggarOutsideTempleTwo {
    ...
    private static int[] bestSolution(int n, int[][] q) {
        ...
        for(int i=0; i<q.length; i++) {
            int start = q[i][0] -1; // for zero index
            int end = q[i][1] -1;
            int value = q[i][2];

            ans[start] = ans[start] + value; // duplicate queries, so add the existing ans[start]
            if(end+1 < n) {
                ans[end+1] = ans[end+1] - value; // duplicate queries, so add the existing ans[end+1]
            }
        }
