...
public class FlattenNestedListIterator {
    ...
    class NestedInteger {
        ...
        ArrayList<NestedInteger> getList() {
            return new ArrayList<NestedInteger>(); // dummy return value
        }
    }
    ...
    class NestedIterator {
        ...
        public NestedIterator(ArrayList<NestedInteger> nestedList) {
            recursiveFunctionToFlattenList(nestedList);
        }

        void recursiveFunctionToFlattenList(ArrayList<NestedInteger> nestedList) {

            for(NestedInteger ele: nestedList) {
                if(ele.isInteger()) {
                    flattenedList.add(ele.getInteger());
                } else {
                    recursiveFunctionToFlattenList(ele.getList());
                }
            }
        }

        int next() {
            int nextEle = flattenedList.get(pointer);
            pointer++;
            return nextEle;
        }

        boolean hasNext() {
            return pointer < flattenedList.size();
        }
    }
}
