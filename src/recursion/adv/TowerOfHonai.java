package recursion.adv;

import java.util.ArrayList;

/*
    In the classic problem of the Towers of Hanoi, you have 3 towers numbered from 1 to 3 (left to right) and
    A disks numbered from 1 to A (top to bottom) of different sizes which can slide onto any tower.
    The puzzle starts with disks sorted in ascending order of size from top to bottom (i.e., each disk sits on top of an even larger one).
    You have the following constraints:

    Only one disk can be moved at a time.
    A disk is slid off the top of one tower onto another tower.
    A disk cannot be placed on top of a smaller disk.
    You have to find the solution to the Tower of Hanoi problem.
    You have to return a 2D array of dimensions M x 3, where M is the minimum number of moves needed to solve the problem.
    In each row, there should be 3 integers (disk, start, end), where:

    disk - number of disk being moved
    start - number of the tower from which the disk is being moved
    stop - number of the tower to which the disk is being moved
 */
public class TowerOfHonai {

    public static void main(String[] args) {

        System.out.println(towerOfHanoi(3));;
    }

    public static ArrayList<ArrayList<Integer>> towerOfHanoi(int A) {

        ArrayList<ArrayList<Integer>> list = new ArrayList<>();
        int n = A;
        int diskNumber = n;
        //                        src, dest, intermediate
        toh(list, n, diskNumber, 1, 3, 2);

        return list;
    }

    private static void toh(ArrayList<ArrayList<Integer>> list, int n, int diskNumber, int src, int dest, int inter) {

        if(n == 1) {
            ArrayList<Integer> innerList = new ArrayList<>();
            innerList.add(diskNumber);
            innerList.add(src);
            innerList.add(dest);
            list.add(innerList);
            return;
        }

        toh(list, n-1, diskNumber-1, src, inter, dest);
        toh(list, 1, diskNumber, src, dest, inter);
        toh(list, n-1, diskNumber-1, inter, dest, src);
    }
}
